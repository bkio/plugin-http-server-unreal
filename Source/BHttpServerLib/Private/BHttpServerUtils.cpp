...
namespace BHttpServerUtils
{
		...
		private:
			...
			void ReadChunk()
			{
				...
				if (ReadCount <= 0)
				{
					if (ReadCount < 0)
					{
						UE_LOG(LogBHttpServer, Error, TEXT("%s: cannot read data from the connection"), *HTTP_SERVER_FUNC_LINE);
					}
	...
	FString GetURLFromGetRequest(mg_connection* RequestConnection)
	{
		...
		FString Data(mg_get_request_info(RequestConnection)->query_string);
